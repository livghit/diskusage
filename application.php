...
function cpu(): int
{
    return match (PHP_OS_FAMILY) {
        "Darwin"
            => (int) `top -l 1 | grep -E "^CPU" | tail -1 | awk '{ print $3 + $5 }'`,
        "Linux"
            => (int) `top -bn1 | grep -E '^(%Cpu|CPU)' | awk '{ print $2 + $4 }'`,
        "Windows" => (int) trim(`wmic cpu get loadpercentage | more +1`),
        "BSD"
            => (int) `top -b -d 2| grep 'CPU: ' | tail -1 | awk '{print$10}' | grep -Eo '[0-9]+\.[0-9]+' | awk '{ print 100 - $1 }'`,
    ...
    ->setCode(function (InputInterface $input, OutputInterface $output): int {
        ...
        render(
            "<p class='text-indigo-500'>" .
                "<span class='text-red-500'>Total CPU Usage at the Moment " .
                $cpuUsage .
                "%</span></p>"
        );
